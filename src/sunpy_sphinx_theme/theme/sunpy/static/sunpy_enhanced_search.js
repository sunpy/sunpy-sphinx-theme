...
(function(root){
	...
	function Search(dialog,config){
		...
		let icons = {
			'filter':'<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M1.5 1.5A.5.5 0 0 1 2 1h12a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.128.334L10 8.692V13.5a.5.5 0 0 1-.342.474l-3 1A.5.5 0 0 1 6 14.5V8.692L1.628 3.834A.5.5 0 0 1 1.5 3.5z"/></svg>',
		...
		if(config.projects){
			...
			document.getElementById('Documentation').querySelectorAll('.nav-link').forEach(function(el){
		...
		style.innerText = `
			...
			#pst-search-dialog[open] form.bd-search { flex-grow: 0; }
			.readthedocs-search {
				background: white;
				border-radius: .25rem;
				width: 100%;
			...
			.readthedocs-search .tablist button {
				display: flex;
				border: 0;
				border-bottom: max(3px,.1875rem,.12em) solid transparent;
			...
			.readthedocs-search .tablist button:focus, .readthedocs-search .tablist button[aria-selected=true] {
				outline: 0;
			...
			.readthedocs-search .results ul li a:hover, .readthedocs-search .results ul li a:focus, .readthedocs-search .results ul li.selected a {
				color: var(--pst-color-text-base);
			...
			.readthedocs-search .credits a { color: inherit; }
			.readthedocs-search .credits svg { height: 20px; }
			@media only screen and (max-width: 70rem) {
				.readthedocs-search .footer, .readthedocs-search .help {
					display: block;
				}
			}
		`;
		...
		for(let p = 0; p < this.projectorder.length; p++){
			...
			if(!this.projects[slug].tab){
				...
				tab.innerHTML = (slug==config.all ? icons.book : icons.filter) + ' ' + this.projects[slug].name + '<span class="n"></span>';
				tab.id = "tab-" + slug;
				...
				tab.addEventListener('click',function(e){ e.preventDefault(); e.stopPropagation(); _obj.selectTab(e); });
				tab.addEventListener('focus',function(e){ e.preventDefault(); e.stopPropagation(); _obj.selectTab(e); });
		...
		this.searchByString = function(str){
			...
			if(str.length >= 2){
				...
				else this.resultdelay = setTimeout(function(){ _obj.getReadTheDocsResults(str); },500);				  
			}else{
				this.displayResults('');
			}
			return this;
		...
		this.getReadTheDocsResults = function(str,page){
			...
			fetch(url,{}).then(response =>{
				...
				if(page){
					...
					_obj.results[str].results = _obj.results[str].results.concat(json.results);
				}else{
		...
		this.getMoreReadTheDocsResults = function(){
			...
			if(data.next) this.getReadTheDocsResults(str,data.next.replace(/%3A/g,":"));
			return this;
		};
		...
		this.formatResult = function(str,data){
			...
			for(let b = 0; b < data.blocks.length; b++){
				...
				if("highlights" in data.blocks[b] && "content" in data.blocks[b].highlights && data.blocks[b].highlights.content.length > 0){
					extract = data.blocks[b].highlights.content[0];

					// Sanitise HTML
					extract = extract.replace(/<span>(.*?)<\/span>/g,function(m,p1){ return '[[MATCH]]'+p1+'[[/MATCH]]'; });
					extract = extract.replace(/<[^\>]+>/g,'');
					extract = extract.replace(/\[\[(\/?)MATCH\]\]/g,function(m,p1){ return '<'+p1+'strong>'; });
		...
		this.displayResults = function(str){
			...
			for(r = 0; r < data.results.length; r++){
				...
				if(slug in this.projects) {
					...
					li.innerHTML = result;
					li.addEventListener('keyup',function(e){ _obj.navLi(e); });
					...
					li2.addEventListener('keyup',function(e){ _obj.navLi(e); });
			...
			for(slug in this.projects){
				...
				this.projects[slug].value.innerHTML = ' (' + this.projects[slug].count + ')';
				this.projects[slug].tab.style.display = (this.projects[slug].count == 0 ? 'none' : '');
			}

			// Update all tabs
			this.projects[config.all].tab.style.display = (data.results.length > 0 ? "" : "none");
			this.projects[config.all].value.innerHTML = (data.results.length > 0 ? ' (' + data.results.length + ')' : '');
			...
			if(data.next){
				...
				this.msg.footer.innerHTML = '<button class="load-more'+(config['load-more'].class ? ' '+config['load-more'].class : '')+'">'+config['load-more'].label+'</button>';
				this.msg.footer.querySelector('button').addEventListener('click',function(e){
					e.preventDefault();
					e.stopPropagation();
			...
			this.msg.header.innerHTML = (str && data.results.length==0 ? "<p"+(config['no-results'].class ? ' class="'+config['no-results'].class+'"' : '')+">"+config['no-results'].label+"</p>" : "");

			return this;
		};

		this.selectTab = function(e,inc){
			let tab = (e.target.tagName.toUpperCase()==="BUTTON" ? e.target : e.target.closest('button'));
			let found = "";
			...
			if(found in this.projects) this.projects[found].tab.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
			return this;
		};
		...
		this.navLi = function(e){
			...
			if(e.key == "ArrowDown"){
				...
				if(prevEl){
					prevEl.focus();
					prevEl.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
				}
			}else if(e.key == "Enter"){
	...
	ready(function(){
		...
		let search = new Search(document.getElementById('pst-search-dialog'),config);
